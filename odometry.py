...
class Odometry:
    ...
    def start_driving(self, angle: float, dt: float):
        ...
        self._x = self._x + v*cos(self._theta + w * dt)
        self._y = self._y + v*sin(self._theta + w * dt)
    ...
    def wheel_speed_to_linear_and_angular(self, wl: float, wr: float):
        return (wl + wr)*self._wheel_radius / 2.0, (wr - wl) * self._wheel_radius / (2.0 * self._wheel_base_half)
